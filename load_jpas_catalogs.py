...
def load_minijpas_jnep(cat_list=['minijpas', 'jnep'], selection=False,
    ...
    for name in cat_list:
        ...
        if flags_mask:
            mask_flagged = (cat.FLAGS == 0) & (cat.MASK_FLAGS == 0)
            print(f'Dropped {sum(mask_flagged)}/{len(mask_flagged)} flagged sources')
            cat = cat[mask_flagged] # Drop flagged
        cat = cat.reset_index()
...
def load_minijpas_jnep_fnu(cat_list=['minijpas', 'jnep'], selection=False,
                           flags_mask=True):
    # If selection, return the valuable items for visual_inspection.py only
    pm_flx = np.array([]).reshape(60, 0)
    pm_err = np.array([]).reshape(60, 0)
    ...
    for name in cat_list:
        cat = pd.read_csv(f'csv/{name}.Fnu_aper3_photoz_gaia_3.csv', sep=',', header=1,
            converters={0: int, 1: int, 2: split_converter, 3: split_converter, 4: sum_flags,
            5: sum_flags})
        ...
        pm_flx_i = np.stack(cat['FLUX_APER_3_0'].to_numpy()).T * 1e-30
        pm_err_i = np.stack(cat['FLUX_RELERR_APER_3_0'].to_numpy()).T * pm_flx_i
...
def load_minijpas_jnep_magab(cat_list=['minijpas', 'jnep'], selection=False,
                           flags_mask=True):
    # If selection, return the valuable items for visual_inspection.py only
    pm_flx = np.array([]).reshape(60, 0)
    pm_err = np.array([]).reshape(60, 0)
    ...
    for name in cat_list:
        cat = pd.read_csv(f'csv/{name}.MagAB_aper3_photoz_gaia_3.csv', sep=',', header=1,
            converters={0: int, 1: int, 2: split_converter, 3: split_converter, 4: sum_flags,
            5: sum_flags})

        cat = cat[np.array([len(x) for x in cat['MAG_APER_3_0']]) != 0] # Drop bad rows due to bad query
        ...
        pm_flx_i = np.stack(cat['MAG_APER_3_0'].to_numpy()).T
        pm_err_i = np.stack(cat['MAG_ERR_APER_3_0'].to_numpy()).T
        ...
        pm_err_i = (pm_err_i ** 2 + Zero_point_error(cat['TILE_ID'], name, which='magAB') ** 2) ** 0.5
