...
lineinfo = fread[1].data[np.where(fread[1].data['LINEWAVE'] == 1549.48)]
...
data = {
    'CIVEW': lya_EW,
    'CIVCont': lya_cont,
    'CIVCont_err': lya_cont_err
}

pd.DataFrame(data).to_csv('CIV_fts.csv')
...
fread = fits.open('fits/spAllLine-v5_13_0.fits')
lineinfo = fread[1].data[np.where(fread[1].data['LINEWAVE'] == 1908.734)]
...
data = {
    'CIIIEW': lya_EW,
    'CIIICont': lya_cont,
    'CIIICont_err': lya_cont_err
}

pd.DataFrame(data).to_csv('CIII_fts.csv')
...
fread = fits.open('fits/spAllLine-v5_13_0.fits')
lineinfo = fread[1].data[np.where(fread[1].data['LINEWAVE'] == 2800.31519)]
